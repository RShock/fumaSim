    ...
    public void initSkills() {
        ...
        skill = BaseSkill.builder().name("【系统级规则】第一回合").trigger(TriggerEnum.游戏开始时)
                ...
                .cast(pack -> {
                    ...
                    GameBoard.getAlly().forEach(chara -> chara.setStatus(BaseChara.CharaStatus.ACTIVE));
        ...
        skill = BaseSkill.builder().name("【系统级规则】角色行动完进入下一回合").trigger(TriggerEnum.角色行动结束)
                .time(INFI)
                .check(pack -> GameBoard.getAlly().stream().noneMatch(chara -> chara.getStatus().equals(BaseChara.CharaStatus.ACTIVE)))
                .cast(pack -> {
